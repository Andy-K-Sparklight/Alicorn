...
async function unpackOne(lib: string, out: string, exclude?: string[]): Promise<void> {
    ...
    try {
        ...
        const files = Object.values(await f.entries()).filter(
            ent => !ent.isDirectory && filter(ent.name),
        );
...
async function unpack(
    ...
    const sources = profile.libraries.filter(
        l => filterRules(l.rules, features) && nativeLib.isNative(l),
    );
    ...
    await Promise.all(
        sources.map(async s => {
            ...
            if (name) {
                await unpackOne(
                    container.nativeLibrary(s.name, name),
                    nativesRoot,
                    s.extract?.exclude,
                );
            }
