...
test("Load Vanilla Profile (1.7.10)", async () => {
    ...
    expect(p.downloads.client.sha1, "Should load client artifact details").toEqual(
    ...
    expect(p.logging?.client.file.id, "Should load logging configuration").toEqual(
        "client-1.7.xml",
    );
});
...
test("Load Vanilla Profile (1.16.5)", async () => {
    ...
    expect(p.downloads.client_mappings?.sha1, "Should load client mappings").toEqual(
...
test("Load Inherited Profile (1.7.10 Forge)", async () => {
    ...
    expect(p.arguments.game[0], "Should load arguments").toEqual("--username");
    expect(p.libraries[0].name, "Should load libraries").toEqual(
...
test("Load Inherited Profile (1.20.1 Fabric)", async () => {
    ...
    expect(p.mainClass, "Should load main class").toEqual(
    ...
    expect(p.arguments.jvm[0], "Should load VM arguments").toEqual(
