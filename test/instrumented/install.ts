...
await iTest.run(
    ...
    async () => {
        ...
        assert(
            [m.latest.release, m.latest.snapshot].includes(m.versions[0].id),
            "Version list should be sorted",
        );
    },
    "medium",
);
...
await iTest.run(
    ...
    async () => {
        ...
        assert(
            versions.map(v => v.toLowerCase()).some(v => v.includes("forge")),
            "Should found Forge installation",
        );
    },
    "full",
);
...
await iTest.run(
    ...
    async () => {
        ...
        assert(
            await fs.pathExists(c.assetIndex(pf.assetIndex.id)),
            "Asset index file should exist",
        );
